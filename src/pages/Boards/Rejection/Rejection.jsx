 ...
 console.log(edit)
  ...
  const renderPartsSection = (partData, dayData, showStaticText = false) => {
    ...
    const values = [
      ...
      dayData?.runner || '-',
      dayData?.isssued || '-',
      dayData ? ((dayData.rejections || 0) + (dayData.lumps || 0) + (dayData.runner || 0)).toFixed(2) : '-'
    ];
    ...
    return (
      <Box sx={{ minHeight: '140px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
  ...
  const renderTotalSection = (partData) => {
    const totals = [
      ...
      ((partData.total.rejections || 0) + (partData.total.lumps || 0) + (partData.total.runner || 0)).toFixed(2)
    ];
  ...
  const handleUpdate= async()=>{
    try {
      ...
      const response = await axios.put(`${BACKEND_API}/update_rejection_detail/${edit.id}`, updatePayload, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
                ...
                {processedData.map((partData, index) => (
                  <TableRow key={`${partData.part_description}-${partData.rm}`} hover>
                    ...
                    {days.map((day) => {
                      ...
                      const isEditing = edit.part_description === partData.part_description && edit.day === day;
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  type="number"
                  value={edit.ok_parts || ''}
                  onChange={(e) => setEdit(prev => ({ ...prev, ok_parts: parseFloat(e.target.value) || 0 }))}
                  variant="outlined"
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={(e) => setEdit(prev => ({ ...prev, rejections: parseFloat(e.target.value) || 0 }))}
                  variant="outlined"
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  type="number"
                  value={edit.lumps || ''}
                  onChange={(e) => setEdit(prev => ({ ...prev, lumps: parseFloat(e.target.value) || 0 }))}
                  variant="outlined"
                  inputProps={{ min: 0, step: 0.1 }}
                />
              </Grid>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  type="number"
                  value={edit.runner || ''}
                  onChange={(e) => setEdit(prev => ({ ...prev, runner: parseFloat(e.target.value) || 0 }))}
                  variant="outlined"
                  inputProps={{ min: 0, step: 0.1 }}
                />
              </Grid>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  type="number"
                  value={edit.isssued || ''}
                  onChange={(e) => setEdit(prev => ({ ...prev, isssued: parseFloat(e.target.value) || 0 }))}
                  variant="outlined"
                  inputProps={{ min: 0, step: 0.1 }}
                />
              </Grid>
