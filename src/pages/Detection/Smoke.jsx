  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.get(
        ...
        filter == 'day'? `${BACKEND_API}/snapdate_count-daywise/${year}/${month}/${day}?camera_id=${selectedCamera}&category=smoke` : 
        filter== 'month'? `${BACKEND_API}/snapmonth/${year2}/${month2}?camera_id=${selectedCamera}&category=smoke` : 
        // `${BACKEND_API}/snapdate_count-timerange/2025/5/2?category=smoke&start_time=15%3A42&end_time=15%3A43&camera_id=9`,
        `${BACKEND_API}/snapdate_count-timerange/${year}/${month}/${day}?category=smoke&start_time=${startTimeFormatted}&end_time=${endTimeFormatted}&camera_id=${selectedCamera}`, 
...
<Typography fontSize={'1.6rem'} sx={{borderBottom: '1px solid grey', mb: '1rem'}}>Smoke Detection:-</Typography>
{/* -----------------------Show cameras of smoke------------------------------------------ */}
 <Box  mt={'0rem'}  width={'100%'} borderRadius={'6px'} display={'flex'} alignItems={'start'} justifyContent={'space-between'}>
      ...
      <Table aria-label="simple table">
        ...
        <TableBody>
          {
            cameras?.smoke?.length>0?
            cameras?.smoke.map((elem, index)=>(
<TableRow key={index}> 
            ...
            <TableCell align="start"><IconButton onClick={()=> navigate(`/single-stream/smoke/${elem.camera_id}`)}><FaEye /></IconButton></TableCell>
    ...
    <Box onClick={()=> navigate(`/multi-stream/smoke`)} height={'11rem'} width={'13rem'} bgcolor={'white'} mr={'1.5rem'} ml={'1rem'} borderRadius={'8px'} display={'flex'} flexDirection={'column'} alignItems={'center'} justifyContent={'center'} sx={{cursor: 'pointer'}} boxShadow={'rgba(0, 0, 0, 0.14) 0px 3px 8px'}>
      ...
      <Typography position={'absolute'} top={'-1.9rem'} left={0}>{report.length} Records found</Typography>
          <Typography fontSize={'1.6rem'} sx={{borderBottom: '1px solid grey', mb: '1rem'}}>Smoke Report</Typography>
...
mb={'1rem'}
        ...
        >
          ...
          <Typography display={'flex'} alignItems={'center'}><Box height={'10px'} width={'10px'} bgcolor={'green'} mr={'0.5rem'} borderRadius={'50%'}></Box>Running Cameras - {cameras?.smoke?.length} </Typography>
