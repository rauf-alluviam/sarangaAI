...
const Signup = ({ setFlag }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      enqueueSnackbar(response?.data?.message || 'User Registered Successfully', {
        variant: 'success',
      });
      setName('');
      setEmail('');
...
onChange={(e)=> setPhone_no(e.target.value)}
      ...
      <Typography mt={'0.4rem'}>
        Already have an account?{' '}
        <u
          onClick={() => setFlag(true)}
          style={{ cursor: 'pointer', color: 'blue', opacity: '65%' }}
        >
          Login
        </u>{' '}
      </Typography>
