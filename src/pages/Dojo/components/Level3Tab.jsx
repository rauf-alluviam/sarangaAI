...
const Level3Tab = ({
  ...
  level3Error,
  level3FormUploading,
  level3FormUploadError,
  level3FormUploadSuccess,
  onInitializeLevel3,
  onFetchEmployee,
}) => {
  ...
  const [resultStatus, setResultStatus] = useState(
    employee?.Level_3?.pass_fail_status === true
      ? 'passed'
      : employee?.Level_3?.pass_fail_status === false
      ? 'failed'
      : 'not_set'
  );
  ...
  useEffect(() => {
    if (employee?.Level_3) {
      const level3 = employee.Level_3;
      if (level3.pass_fail_status === true) {
        setResultStatus('passed');
      } else if (level3.pass_fail_status === false) {
    ...
    try {
      await handleAsyncError(async () => {
        ...
        const response = await makeApiRequest(
          `${BACKEND_API}/set_level_3_training_result_as_passed_or_failed?user_id=${employee.user_id}&passed=${passed}`,
          {
            method: 'PUT',
          }
        );
  ...
  const handleReassign = async () => {
    ...
    try {
      await handleAsyncError(async () => {
        const response = await makeApiRequest(
          `${BACKEND_API}/reassign_level_3_training?user_id=${employee.user_id}`,
          {
            method: 'POST',
            body: JSON.stringify({}),
          }
  ...
  const level3 = employee?.Level_3;
  const formUploaded = level3?.form_uploaded;
  const horizontal = level3?.['horizontal_injection_molding'] || {};
  const finalInspection = level3?.['final_inspection_packaging'] || {};
            ...
            <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
              ...
              <Box mb={3}>
                ...
                <Box display="flex" alignItems="center" gap={1} flexWrap="wrap">
                  ...
                  <Chip
                    label={`Retrain Count: ${level3?.retrain_count || 0}`}
                    color="info"
                    variant="outlined"
                    size="medium"
                  />
