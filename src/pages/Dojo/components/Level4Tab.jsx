...
const Level4Tab = ({
          ...
          onClick={async () => {
            try {
              ...
              Swal.fire({
                icon: 'error',
                title: 'Error!',
                text: error.message || 'Failed to initialize Level 4 training',
                timer: 3000,
                showConfirmButton: false,
              });
            }
  ...
  const handleLevel3FormUpload = async (event) => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_API}/upload_level_3_form?user_id=${employee.user_id}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          accept: 'application/json',
      ...
      <Box mb={2}>
        <label htmlFor="file-upload-level3-form">
          ...
          <Button
            ...
            sx={{ mt: 1 }}
          >
            {level4FormUploading ? 'Uploading...' : 'Upload Level 4 Evaluation Form'}
          </Button>
        </label>
