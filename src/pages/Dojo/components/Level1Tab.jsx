...
const Level1Tab = ({
  ...
  level1Error,
  level1FormUploading,
  level1FormUploadError,
  level1FormUploadSuccess,
  onInitializeLevel1,
  onFetchEmployee,
}) => {
  ...
  const [resultStatus, setResultStatus] = useState(
    employee?.Level_1?.pass_fail_status === true
      ? 'passed'
      : employee?.Level_1?.pass_fail_status === false
      ? 'failed'
      : 'not_set'
  );
  ...
  useEffect(() => {
    if (employee?.Level_1) {
      ...
      if (level1.pass_fail_status === true) {
        setResultStatus('passed');
      } else if (level1.pass_fail_status === false) {
  ...
  // Level 1 Evaluation Form upload handler
  const handleLevel1FormUpload = async (event) => {
    ...
    try {
      await handleAsyncError(async () => {
        ...
        const response = await fetch(
          `${BACKEND_API}/upload_Level_1_form?user_id=${employee.user_id}`,
          {
            method: 'PUT',
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        await Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: 'Level 1 evaluation form uploaded successfully!',
          timer: 3000,
          showConfirmButton: false,
        });
      }, 'Failed to upload Level 1 evaluation form');
    } finally {
      setUploading(false);
    ...
    try {
      await handleAsyncError(async () => {
        ...
        const response = await makeApiRequest(
          `${BACKEND_API}/set_level_1_training_result_as_passed_or_failed?user_id=${employee.user_id}&passed=${passed}`,
          {
            method: 'PUT',
          }
        );
        ...
        await Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: data.message || 'Level 1 training result updated successfully!',
          timer: 3000,
          showConfirmButton: false,
        });
      ...
      }, 'Failed to set Level 1 training result');
    } catch (error) {
  ...
  const handleReassign = async () => {
    ...
    try {
      await handleAsyncError(async () => {
        const response = await makeApiRequest(
          `${BACKEND_API}/reassign_level_1_training?user_id=${employee.user_id}`,
          {
            method: 'POST',
            body: JSON.stringify({}),
          }
        ...
        await Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: data?.message || 'Level 1 training reassigned successfully',
          timer: 3000,
          showConfirmButton: false,
        });
      ...
      }, 'Failed to reassign Level 1 training');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleInitializeLevel1 = async () => {
    ...
    await handleAsyncError(async () => {
      await onInitializeLevel1();
      await Swal.fire({
        icon: 'success',
        title: 'Success!',
        text: 'Level 1 training initialized successfully!',
        timer: 3000,
        showConfirmButton: false,
      });
    }, 'Failed to initialize Level 1 training');
  };

  // Validation helpers
  const isValidEmployee = employee && employee.user_id;
  const level1 = employee?.Level_1;
  const formUploaded = level1?.form_uploaded;
  const horizontal = level1?.['horizontal_injection_molding'] || {};
  const finalInspection = level1?.['final_inspection_packaging'] || {};
    ...
    return (
      <Paper sx={{ p: 3, minHeight: 320 }}>
        ...
        <Typography color="textSecondary">
          Unable to load Level 1 training data. Please ensure employee information is available.
        </Typography>
      </Paper>
    );
  }

  // Early return for uninitialized Level 1
  if (!level1) {
    return (
      <Paper sx={{ p: 3, minHeight: 320 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <Typography variant="h6">Level 1 Training</Typography>
    ...
    <Paper sx={{ p: 3 }}>
      ...
      <Box mt={3}>
        <Grid container spacing={3}>
          {/* Level 1 Result - Always shown first */}
          <Grid item xs={12}>
            <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
              ...
              <Box mb={3}>
                ...
                <Box display="flex" alignItems="center" gap={1} flexWrap="wrap">
                  ...
                  <Chip
                    label={`Retrain Count: ${level1?.retrain_count || 0}`}
                    color="info"
                    variant="outlined"
                    size="medium"
                  />
