...
const Level2Tab = ({
  ...
  const [resultStatus, setResultStatus] = useState(
    employee?.Level_2?.pass_fail_status === true
      ? 'passed'
      : employee?.Level_2?.pass_fail_status === false
      ? 'failed'
      : 'not_set'
  );
  ...
  useEffect(() => {
    if (employee?.Level_2) {
      const level2 = employee.Level_2;
      if (level2.pass_fail_status === true) {
        setResultStatus('passed');
      } else if (level2.pass_fail_status === false) {
  ...
  const handleLevel2FormUpload = async (event) => {
    ...
    try {
      ...
      }, 'Failed to upload Level 2 evaluation form');
    } finally {
      setUploading(false);
    ...
    try {
      await handleAsyncError(async () => {
        ...
        const response = await makeApiRequest(
          `${BACKEND_API}/set_level_2_training_result_as_passed_or_failed?user_id=${employee.user_id}&passed=${passed}`,
          {
            method: 'PUT',
          }
        );
        ...
        await Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: data.message || 'Level 2 training result updated successfully!',
          timer: 3000,
          showConfirmButton: false,
        });
      ...
      }, 'Failed to set Level 2 training result');
    } catch (error) {
  ...
  const handleReassign = async () => {
    ...
    try {
      await handleAsyncError(async () => {
        const response = await makeApiRequest(
          `${BACKEND_API}/reassign_level_2_training?user_id=${employee.user_id}`,
          {
            method: 'POST',
            body: JSON.stringify({}),
          }
        ...
        await Swal.fire({
          icon: 'success',
          title: 'Success!',
          text: data?.message || 'Level 2 training reassigned successfully',
          timer: 3000,
          showConfirmButton: false,
        });
      ...
      }, 'Failed to reassign Level 2 training');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleInitializeLevel2 = async () => {
    ...
    }, 'Failed to initialize Level 2 training');
  };

  // Validation helpers
  const isValidEmployee = employee && employee.user_id;
  const level2 = employee?.Level_2;
  const formUploaded = level2?.form_uploaded;
  const horizontal = level2?.['horizontal_injection_molding'] || {};
  const finalInspection = level2?.['final_inspection_packaging'] || {};
    ...
    return (
      <Paper sx={{ p: 3, minHeight: 320 }}>
        ...
        <Typography color="textSecondary">
          Unable to load Level 2 training data. Please ensure employee information is available.
        </Typography>
      </Paper>
    );
  }

  // Early return for uninitialized Level 2
  if (!level2) {
    return (
      <Paper sx={{ p: 3, minHeight: 320 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <Typography variant="h6">Level 2 Training</Typography>
    ...
    <Paper sx={{ p: 3 }}>
      ...
      <Box mt={3}>
        <Grid container spacing={3}>
          {/* Level 2 Result - Always shown first */}
          <Grid item xs={12}>
            <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
              ...
              <Box mb={3}>
                ...
                <Box display="flex" alignItems="center" gap={1} flexWrap="wrap">
                  ...
                  <Chip
                    label={`Retrain Count: ${level2?.retrain_count || 0}`}
                    color="info"
                    variant="outlined"
                    size="medium"
                  />
